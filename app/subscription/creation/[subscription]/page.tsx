...
export default function Page({ params }: { params: { subscription: string } }) {
  ...
  const createSubscription = async () => {
    const data: any = await getTenantDetails(params?.subscription);
    if (
      data[0].credentials == null &&
      data[0].details == null &&
      data[0].subscriptionId == null
    ) {
      const req: any = await beginTenantCreation(params?.subscription);
      setCompleted(true);
      router.push(`tenant/${req.data.tenant}`);
    } else {
      setTimeout(() => {
        router.push(`tenant/${data[0]?.id}`);
      }, 1000);
    }
  };
